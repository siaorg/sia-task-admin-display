        ...
        <div class="section-content">
            <el-collapse v-model="activeNames" accordion @change="handleChangeList">
                <el-collapse-item v-for="(item,index) in titleListCount" :key="index" :name="item.groups" v-if="item.groups!==''">
                    ...
                    <el-table :data="viewJobLogManageList" class="log-manage-table">
                            ...
                            <template slot-scope="scope" v-if="scope.row.taskLogList!==0">
                                <div class="task-info" v-for="(taskInfoList, index) in scope.row.taskLogList" :key="index">
                                    ...
                                    <div class="task-desc">
                                        ...
                                        <span v-show="taskInfoList.taskHandleTime!==null"><i>执行完成时间：</i><i>{{taskInfoList.taskHandleTime | formatTime}}</i></span>
                        ...
                        <el-table-column label="Job_Key" width="120" prop="jobKey" align="center">
                            <template slot="header" slot-scope="scope">
                                <el-popover placement="bottom-start" popper-class="select-box-popver" :ref="item.groups" trigger="click">
                        ...
                        <el-table-column label="日志详情" prop="id">
                            <template slot-scope="scope">
                                <p class="job-detail">
                                    ...
                                    <span v-show="scope.row.jobHandleCode!==null"><i>开始调度时间:</i><i>{{scope.row.jobTriggerTime | formatTime}}</i></span>
                                    <span v-show="scope.row.jobHandleFinishedTime!==null"><i>调度完成时间:</i><i>{{scope.row.jobHandleFinishedTime | formatTime}}</i></span>
...
export default {
    ...
    methods: {
        ...
        showHiddenRefreshTaskList: function () {
            ...
            this.getSearchTitleList(this.$store.state.DeskTtopStore.curAppGroup)
            ...
            self.$http.get(self.$api.getApiAddress('/logapi/countGroupsJobLogs', 'CESHI_API_HOST'), {
                ...
                if (res.data.code === 0) {
                    ...
                    res.data.data.forEach(item => {
                        if (item.groups === this.$store.state.DeskTtopStore.curAppGroup) {
                            temArr.push(item)
                        }
                    })
                    self.titleListCount = temArr
                    if (val !== '') {
                        self.getJobLogList(self.pageSize, self.currentPageIndex, self.jobGroupName, this.jobKeyName)
                    }
                } else {
        ...
        getSearchList: function () {
            ...
            //this.jobGroupName = this.$store.state.DeskTtopStore.curAppGroup
        },
