        ...
        <div class="info">
          <el-form :model="taskAddViewModel" :rules="taskAddViewModelRules" ref="taskManageViewForm" label-width="120px" class="taskManageViewForm" auto-complete="off">
            <el-form-item label="项目组名称" prop="projectName">
              <!-- <el-input type="text" auto-complete="off" v-model="$store.state.DeskTtopStore.curAppGroup" disabled></el-input> -->
            ...
            <el-form-item label="应用名称" prop="applyName">
              ...
              <span class="pre-job-key-text">{{$store.state.DeskTtopStore.curAppGroup}}</span><span class="center-line">－</span>
              <el-select class="pre-task-key" placeholder="应用名称" v-model="taskAddViewModel.applyName" :allow-create="true" :filterable="true">
...
export default {
  ...
  data () {
    var checkBlur = (rule, value, callback) => {
      switch (rule.field) {
        case 'httpPath':
          if (!new RegExp(/^\/([\/a-z_A-Z-.0-9]+)$/).test(value)) {
            return callback(new Error('HTTP_PATH格式必须以 " / "开头，其余字符可为英文、数字、下划线、中划线和.'))
          }
          break
        case 'taskdesc':
  ...
  methods: {
    getSearchList: function () {
      this.projectDataList = [this.$store.state.DeskTtopStore.curAppGroup]
      this.taskAddViewModel.projectName = this.projectDataList[0]
    },
      ...
      self.$http.get(self.$api.getApiAddress('/taskapi/selectappsbygroup', 'CESHI_API_HOST'), {
        ...
        if (res.data.code !== 0) {
          ...
          res.data.data.forEach((ele) => {
            self.applyDataList.push(ele.slice((ele.indexOf('-') + 1), ele.length))
          })
