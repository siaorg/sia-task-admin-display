...
export default {
  ...
  methods: {
    ...
    clickSaveTaskConfig () {
      ...
      // task_key
      // 判断是否有参数为空
      let isSavePrams = true
      params.forEach(function (val) {
        // paramCount为0的时候没有参数
        if (val.failover === null || val.routeStrategy === null) {
          self.$message({message: 'TASK配置项不能为空', type: 'error'})
          isSavePrams = false
        }
        if (Object.keys(val).indexOf('failover') === -1 || Object.keys(val).indexOf('routeStrategy') === -1) {
          self.$message({message: 'TASK配置项不能为空', type: 'error'})
          isSavePrams = false
        }
      ...
      params.forEach(function (val) {
        ...
        if (val.taskId.indexOf('jsPlumb_') !== -1) {
          val.taskId = val.taskId.split('_')[1]
        }
        if (val.preTaskKey instanceof Array) {
    ...
    getTaskKeyList: function () {
      ...
      return new Promise((resolve) => {
        ...
        self.$http.get(self.$api.getApiAddress('/taskapi/selectTasks', 'CESHI_API_HOST'), {
          ...
          if (res.data.code === 0) {
            this.taskGrounpList = Object.keys(res.data.data)
            ...
            self.leftMenuList = tmpArr
            self.leftMenuTotalList = tmpArr
