<template>
    <div class="dispatch-system-default job-manage-list-page"
      ...
      <div class="section-header">
        <el-input v-model="descriptionInput" prefix-icon="el-icon-search" placeholder="请输入描述内容" style="width:400px;margin-right:10px;"></el-input>
        <el-button class="btn-large edit-btn" @click="handleSearch">查询</el-button>
        ...
        <el-button class="btn-large edit-btn btn-ml-auto" @click="handleClickAddJob"> 添加Job </el-button>
      </div>
        ...
        <el-collapse v-model="activeNames" accordion @change="handleChangeList">
          <el-collapse-item v-for="(item,index) in jobGroupTitleList" :key="index" :name="item.jobGroupName" v-if="item.jobGroupName!==''">
            ...
            <el-table :data="viewJobManageList" style="width: 100%" class="task-manage-table">
              ...
              <el-table-column show-overflow-tooltip prop="jobDesc" label="描述" width="150" align="center">
              </el-table-column>
              ...
              <el-table-column label="操作" width="240">
                <template slot-scope="scope">
                  ...
                  <el-button class="btn-radius edit-btn ml" @click="handleClickSetCasca(scope.row, item.jobNum)" :disabled="true"> 级联设置 </el-button>
                </template>
              </el-table-column>
              <template slot="empty">
                <p class="no-data">
...
export default {
  ...
  created () {
    this.loading = true
    this.getListProject(this.$store.state.DeskTtopStore.curAppGroup)
    this.getSearchList()
  },
  methods: {
    handleSearch() {
      this.getJobList(this.activeNames, '', this.pageSize, this.currentPageIndex, this.descriptionInput)
    },
    // jobkey搜索框
    ...
    getListProject: function (val) {
      ...
      self.$http.get(self.$api.getApiAddress('/jobapi/selectGroupAndJobCount', 'CESHI_API_HOST'), {
        jobGroupName: this.$store.state.DeskTtopStore.curAppGroup
      }).then((res) => {
        if (res.data.code === 0) {
          self.jobGroupTitleList = res.data.data
          if (val !== '' && val !== '全部') {
            self.getJobList(this.$store.state.DeskTtopStore.curAppGroup, this.jobKeyName, self.pageSize, self.currentPageIndex)
          } else {
            this.loading = false
          }
        } else {
    ...
    getSearchList: function () {
      let self = this
      self.$http.get(self.$api.getApiAddress(`/jobapi/selectJobKeysByGroup?jobGroupName=${this.$store.state.DeskTtopStore.curAppGroup}`, 'CESHI_API_HOST')).then((res) => {

        self.viewSearchDateList = {}
        self.viewSearchDateList[this.$store.state.DeskTtopStore.curAppGroup] = res.data.data
        ...
        if (self.jobGroupList.length !== 0) {
          ...
          self.jobGroupName = this.$store.state.DeskTtopStore.curAppGroup
        } else {
          self.$message({message: res.data.message, type: 'error'})
        }
      }).catch(() => {
    ...
    getJobList(jobGroup, jobKey, pageSize, currentPageIndex, jobDesc ='') {
      let self = this
      self.$http.get(self.$api.getApiAddress('/jobapi/selectjobs?jobGroupName=' + jobGroup + '&' + 'jobKey=' + jobKey + '&' + 'jobDesc=' + jobDesc + '&' + 'pageSize=' + pageSize + '&' + 'currentPage=' + currentPageIndex, 'CESHI_API_HOST')).then((res) => {
        if (res.data.code === 0) {
          self.viewJobManageList = res.data.data.items
          self.pageCount = res.data.data.totalPage
          self.currentPageIndex = currentPageIndex - 0
          if (jobGroup !== '全部') {
