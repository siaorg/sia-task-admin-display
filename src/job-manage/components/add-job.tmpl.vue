...
export default {
  ...
  data () {
    var checkBlur = (rule, value, callback) => {
      switch (rule.field) {
        case 'emailWarning':
          if (value.replace(/(^\s*)|(\s*$)/g, '') !== '' && !new RegExp(/^[^]{1,256}$/).test(value)) {
            return callback(new Error('输入内容不能超过256个字符!'))
          }
          break
          break
        case 'jobKey':
  ...
  methods: {
    ...
    getSearchList: function () {
      this.jobGroupList = [this.$store.state.DeskTtopStore.curAppGroup]
      this.taskAddViewModel.jobGroupName = this.jobGroupList[0]
      // this.$refs.jobManageViewForm.validate(this.taskAddViewModel.jobGroupName)
    },
